...
/* -
name = "Aho Corasick"
source = "https://github.com/kth-competitive-programming/kactl/blob/main/content/strings/AhoCorasick.h"
[info]
...
time = "construction takes $O(26N)$, where $N = \"sum of length of patterns\"$. `find(x)` is $O(N)$, where $N = \"length of\" x$. `findAll` is $O(N M)$."
- */
struct AhoCorasick {
  ...
  AhoCorasick(vec<string>& pat) : N(1, -1) {
  ...
  vec<vi> findAll(vec<string>& pat, string word) {
    vi r = find(word);
    vec<vi> res(sz(word));
    rep(i,0,sz(word)) {
      int ind = r[i];
