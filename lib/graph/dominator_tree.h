...
/* -
name = "Dominator Tree"
source = "https://github.com/ucf-programming-team/hackpack-cpp/blob/master/content/graphs/DominatorTree.h"
[info]
description = "Given a digraph, return the edges of the dominator tree given as an adjacency list (directed tree downwards from the root)."
time = "$O((V + E) log V)$"
- */
vec<vi> dominator_tree(const vec<vi>& adj, int root) {
  int n = sz(adj) + 1, co = 0;
  vec<vi> ans(n), radj(n), child(n), sdomChild(n);
