...
[info]
description = "Halfplanes are defined by a vector starting from p with direction pq, covering the left of this line."
time = "$O(n log n)$"
- */
...
vec<P> halfPlaneIntersection(vec<Halfplane>& H) { 
  ...
  rep(i,0,sz(H)) {
    ...
    if (len > 0 && fabsl(H[i].pq.cross(dq[len-1].pq)) < eps) {
      if (H[i].pq.dot(dq[len-1].pq) < 0.0) return vec<P>();
